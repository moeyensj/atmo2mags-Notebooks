...
% some useful info harvested from Joachim's emails

% the color coding of symbols in plots 
Kurucz MS: metallicity 
Galaxies: redshift bins between 0.0 and 3.0 
Supernovae: redshift bins between 0.0 and 1.2
Quasars: redshift bins between 0.0 and 7.5
MLT dwarfs: no color scheme
White dwarfs: no color scheme


for delta-delta plots (right column): 
Kurucz MS - Dots: Cyan (-5 <= M < -3), Blue (-3 <= M < -2) , Green (-2 <= M < -1),
       Yellow (-1 <= M < 0), Red (0 <= M < 1), Purple ( 1 <= M) where M is metallicity
MLT Dwarfs - Crosses: Blue always
White Dwarfs - Crosses: Yellow always
Galaxies - Dots: Blue (0.0 <=z < 1.0), Green (1.0 <= z < 2.0), Red (2.0 <= z < 3.0), Purple ( 3.0 <= z)
Quasars - Circles: Blue (0.0 <= z < 1.0), Green (1.0 <= z < 2.0), Red (2.0 <= z < 3.0), Purple ( 3.0 <= z <= 7.5)***
Supernova - Squares: Blue (0.0 <= z < 0.4), Green (0.4 <= z < 0.8), Red (0.8 <= z <= 1.2)
